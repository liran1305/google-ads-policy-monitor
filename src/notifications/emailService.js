...
Generated by Google Ads Policy Monitor
  ...
  generateContentDiff(previousContent, currentContent, url = '') {
    ...
    const previousSentences = normalizePrevious.split(/[.!?]+/).filter(s => s.trim().length > 10);
    const currentSentences = normalizeCurrent.split(/[.!?]+/).filter(s => s.trim().length > 10);
    
    // Find added and removed sentences
    const addedSentences = currentSentences.filter(sentence => 
      !previousSentences.some(prevSent => 
        this.sentenceSimilarity(sentence.trim(), prevSent.trim()) > 0.8
      )
    );
    
    const removedSentences = previousSentences.filter(sentence => 
      !currentSentences.some(currSent => 
        this.sentenceSimilarity(sentence.trim(), currSent.trim()) > 0.8
      )
    );
